...
app.config['MYSQL_PASSWORD'] = '0000'##Use mysql root password
...
def update_password(username):
	...
	if request.method == 'POST' and form.validate():
		...
		if sha256_crypt.verify(entered, old):
			cur.execute("UPDATE admins SET password = %s WHERE username = %s", (sha256_crypt.encrypt(new), username))
			mysql.connection.commit()
			cur.close()
			flash('New password will be in effect from next login!!', 'admins')
...
def addTrainor():
	values.clear()
	cur = mysql.connection.cursor()
	q = cur.execute("SELECT username FROM admins")
	b = cur.fetchall()
	for i in range(q):
		values.append(b[i]['username'])
	#app.logger.admins(b[0]['username'])
	#res = values.fetchall()
	#app.logger.admins(res)
	cur.close()
	form = AddTrainorForm(request.form)
	if request.method == 'POST' and form.validate():
		#app.logger.admins("setzdgxfhcgjvkhbjlkn")
		name = form.name.data
	...
	#	password = sha256_crypt.encrypt(str(form.password.data))
		...
		cur.execute("INSERT INTO admins(name, username, password, street, city, prof, phone) VALUES(%s, %s, %s, %s, %s, %s, %s)", (name, username, password, street, city, 3,phone))
...
def deleteTrainor():
	...
	if len(choices)==1:
		...
		return redirect(url_for('adminDash'))
	if request.method == 'POST':
		#app.logger.admins(form.username.data)
		username = form.username.data
...
def addRecep():
	...
	if request.method == 'POST' and form.validate():
		...
		cur.execute("INSERT INTO admins(name, username, password, street, city, prof, phone) VALUES(%s, %s, %s, %s, %s, %s, %s)", (name, username, password, street, city, 2,phone))
...
def addMember():
	...
	if request.method == 'POST' and form.validate():
		...
		cur.execute("INSERT INTO admins(name, username, password, street, city, prof, phone) VALUES(%s, %s, %s, %s, %s, %s, %s)", (name, username, password, street, city, 4,phone))
...
def viewDetails():
	cur = mysql.connection.cursor()
	cur.execute("SELECT username FROM admins WHERE username != %s", [session['username']])
	result = cur.fetchall()
...
def trainorDash():
	...
	if request.method == 'POST':
		...
		if rate == 'good':
			...
			rate = 3
		#app.logger.admins(request.form.input_date)
		#app.logger.admins(date)
		...
		cur.close()
		choices.clear()
		flash('Progress updated and Reported', 'admins')
		return redirect(url_for('trainorDash'))
...
def memberDash(username):
	...
	if total==0:
		flash('The user has no progress yet', 'admins')
		return render_template('home.html')
...
def edit_profile(username):
	...
	if request.method == 'POST' and form.validate():
		...
		app.logger.admins(name)
		app.logger.admins(street)
		app.logger.admins(city)
		cur = mysql.connection.cursor()

		q = cur.execute("UPDATE admins SET name = %s, street = %s, city = %s, phone = %s WHERE username = %s", (name, street, city, phone, username))
		app.logger.admins(q)
		mysql.connection.commit()
		cur.close()
